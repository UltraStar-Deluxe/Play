...
{
    private static readonly string folderPath = $"{Application.dataPath}/Tests/EditModeTests/FileScannerTest/";
    ...
    {
        List<string> files = FileScanner.GetFiles(folderPath.Replace("/", "//"),
            new FileScannerConfig(txtFilePattern) { ExcludeHiddenFiles = false, ExcludeHiddenFolders = true, Recursive = true });
        Assert.AreEqual(4, files.Count);
    }
