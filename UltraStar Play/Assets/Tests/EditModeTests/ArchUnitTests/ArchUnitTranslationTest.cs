...
public class ArchUnitTranslationTest
{
    ...
    private static void LoadIgnoredUntranslatedAssignments()
    {
        ...
        string[] lines = File.ReadAllLines("Assets/Tests/EditModeTests/ArchUnitTests/IgnoredUntranslatedAssignments.csv");
        if (lines.Length > 1)
        {
    ...
    {
        string[] lines = File.ReadAllLines("Assets/Tests/EditModeTests/ArchUnitTests/IgnoredUntranslatedEnums.csv");
        if (lines.Length > 1)
        {
            Debug.LogWarning($"Ignoring untranslated assignments:\n  {lines.JoinWith("\n  ")}");
        }
