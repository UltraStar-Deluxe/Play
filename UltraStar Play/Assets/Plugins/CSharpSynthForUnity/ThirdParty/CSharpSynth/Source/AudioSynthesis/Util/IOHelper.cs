namespace AudioSynthesis.Util
{
    ...
    public static class IOHelper
    {
		...
		public static char Read8BitChar(byte[] reader, ref int index)
		{
		...
		public static char[] Read8BitChars(byte[] reader, int length, ref int index)
		{
			char[] chars = new char[length];
			for (int x = 0; x < chars.Length; x++)
				chars[x] = (char)reader[index++];
			return chars;
		}
    ...
    {
        // Integer type helpers
		public static short ReadInt16(byte[] input, ref int index)
        {
            return (short)((input[index++] << 8) | input[index++]);
        }
        public static short ReadInt16(BinaryReader reader)
        {
            return (short)((reader.ReadByte() << 8) | reader.ReadByte());
        }
		public static int ReadInt24(byte[] input, ref int index)
        {
            return ((((input[index++] << 16) | (input[index++] << 8) | input[index++]) << 12) >> 12);
        }
        public static int ReadInt24(BinaryReader reader)
        {
            return ((((reader.ReadByte() << 16) | (reader.ReadByte() << 8) | reader.ReadByte()) << 12) >> 12);
        }
        public static int ReadInt32(byte[] input, ref int index)
        {
			return ((input[index++] << 24) | (input[index++] << 16) | (input[index++] << 8) | input[index++]);
        }
        public static int ReadInt32(BinaryReader reader)
        {
