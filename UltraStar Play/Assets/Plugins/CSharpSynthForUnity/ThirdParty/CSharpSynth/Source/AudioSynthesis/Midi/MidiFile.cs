...
namespace AudioSynthesis.Midi
{
	public class MidiFile
    {
		...
		private void LoadStream(byte[] reader)
		{
			int p = 0;
			if (!FindHead(reader, 500, ref p))
				throw new Exception("Invalid midi file : MThd chunk could not be found.");
			ReadHeader(reader, ref p);
			try
			{
				for (int x = 0; x < mTracks.Length; x++)
				{
					mTracks[x] = ReadTrack(reader, ref p);
				}
			}
			catch(EndOfStreamException ex)
			{
		...
		private void ReadHeader(byte[] reader, ref int p)
		{
			if (BigEndianHelper.ReadInt32(reader, ref p) != 6) //midi header should be 6 bytes long
				throw new Exception("Midi header is invalid.");
			mTrackFormat = (TrackFormat)BigEndianHelper.ReadInt16(reader,ref p);
			mTracks = new MidiTrack[BigEndianHelper.ReadInt16(reader, ref p)];
			int div = BigEndianHelper.ReadInt16(reader, ref p);
			mDivision = div & 0x7FFF;
			mTimeFormat = ((div & 0x8000) > 0) ? TimeFormat.FamesPerSecond : TimeFormat.TicksPerBeat;
		}
		...
		private static MidiTrack ReadTrack(byte[] reader, ref int p)
		{
			List<byte> instList = new List<byte>();
			List<byte> drumList = new List<byte>();
			List<MidiEvent> eventList = new List<MidiEvent>();
			...
			int noteOnCount = 0;
			int totalTime = 0;
			while (!new string(IOHelper.Read8BitChars(reader, 4, ref p)).Equals("MTrk"))
			{
				int length = BigEndianHelper.ReadInt32(reader, ref p);
				while (length > 0)
				{
					length--;
					p++;
			...
			long endPosition = BigEndianHelper.ReadInt32(reader, ref p) + p;
			byte prevStatus = 0;
			while (p < endPosition)
			{
				int delta = ReadVariableLength(reader, ref p);
				totalTime += delta;
				byte status = reader[p++];
				if (status >= 0x80 && status <= 0xEF)
				{//voice message
					prevStatus = status;
					eventList.Add(ReadVoiceMessage(reader, delta, status, reader[p++], ref p));
					TrackVoiceStats(eventList[eventList.Count - 1], instList, drumList, ref channelList, ref noteOnCount);
				}
				else if (status >= 0xF0 && status <= 0xF7)
				{//system common message
					prevStatus = 0;
					eventList.Add(ReadSystemCommonMessage(reader, delta, status, ref p));
				}
				else if (status >= 0xF8 && status <= 0xFF)
				{//realtime message
					eventList.Add(ReadRealTimeMessage(reader, delta, status, ref p));
				}
				else
				{//data bytes
					if (prevStatus == 0)
					...
					{//otherwise apply running status
						eventList.Add(ReadVoiceMessage(reader, delta, prevStatus, status, ref p));
						TrackVoiceStats(eventList[eventList.Count - 1], instList, drumList, ref channelList, ref noteOnCount);
					}
				}
			}
		...
		private static MidiEvent ReadMetaMessage(byte[] reader, int delta, byte status, ref int p)
		{
			...
			case 0x0://sequence number
				{
					...
					if (count == 0)
						...
						return new MetaNumberEvent(delta, status, metaStatus, BigEndianHelper.ReadInt16(reader,ref p));
					else
						throw new Exception("Invalid sequence number event.");
				}
			case 0x1://text
				return new MetaTextEvent(delta, status, metaStatus, ReadString(reader, ref p));
			case 0x2://copyright
				return new MetaTextEvent(delta, status, metaStatus, ReadString(reader, ref p));
			case 0x3://trackname
				return new MetaTextEvent(delta, status, metaStatus, ReadString(reader, ref p));
			...
			case 0x51://tempo
				if (reader[p++] != 3)
					throw new Exception("Invalid tempo event. Expected size of 3.");
				return new MetaNumberEvent(delta, status, metaStatus, (reader[p++] << 16) | (reader[p++] << 8) | reader[p++]);
			case 0x54://smpte
				if (reader[p++] != 5)
					throw new Exception("Invalid smpte event. Expected size of 5.");
				return new MetaTextEvent(delta, status, metaStatus, reader[p++] + ":" + reader[p++] + ":" + reader[p++] + ":" + reader[p++] + ":" + reader[p++]);
			case 0x58://time sig
				if (reader[p++] != 4)
					throw new Exception("Invalid time signature event. Expected size of 4.");
				return new MetaTextEvent(delta, status, metaStatus, reader[p++] + ":" + reader[p++] + ":" + reader[p++] + ":" + reader[p++]);
			case 0x59://key sig
				if (reader[p++] != 2)
					throw new Exception("Invalid key signature event. Expected size of 2.");
				return new MetaTextEvent(delta, status, metaStatus, reader[p++] + ":" + reader[p++]);
			case 0x7F://seq specific
				int lenght = ReadVariableLength (reader, ref p);
		...
		private static MidiEvent ReadRealTimeMessage(byte[] reader, int delta, byte status, ref int p)
		{
			switch (status)
			{
			case 0xF8://midi clock
			...
			case 0xFE://active sense
				return new RealTimeEvent(delta, status, 0, 0);
			case 0xFF://meta message
				return ReadMetaMessage(reader, delta, status, ref p);
			default:
				throw new Exception("The real time message was invalid or unsupported : " + status);
			}
		}
		...
		private static MidiEvent ReadSystemCommonMessage(byte[] reader, int delta, byte status, ref int p)
		{
			switch (status)
			{
			case 0xF7://sysEx (either or)
			case 0xF0://sysEx
				{
					short maker = reader[p++];
					if (maker == 0x0)
						maker = BigEndianHelper.ReadInt16(reader, ref p);
					else if (maker == 0xF7)
						return null;
					List<byte> data = new List<byte>();
					byte b = reader[p++];
			...
			case 0xF1://mtc quarter frame
				return new SystemCommonEvent(delta, status, reader[p++], 0);
			case 0xF2://song position
				return new SystemCommonEvent(delta, status, reader[p++], reader[p++]);
			case 0xF3://song select
				return new SystemCommonEvent(delta, status, reader[p++], 0);
			case 0xF6://tune request
				return new SystemCommonEvent(delta, status, 0, 0);
		...
		private static MidiEvent ReadVoiceMessage(byte[] reader, int delta, byte status, byte data1, ref int p)
		{
			switch (status & 0xF0)
			{
			case 0x80: //NoteOff
				return new MidiEvent(delta, status, data1, reader[p++]);
			case 0x90: //NoteOn
				byte velocity = reader[p++];
				if (velocity == 0) //actually a note off event
					status = (byte)((status & 0x0F) | 0x80);
				return new MidiEvent(delta, status, data1, velocity);
			case 0xA0: //AfterTouch
		...
		private static int ReadVariableLength(byte[] reader, ref int index)
		{
			int value = 0;
			int next;
			do
		...
		private static string ReadString(byte[] reader, ref int p)
		{
			
			int length = ReadVariableLength(reader, ref p);
			byte[] String = new byte[length];
			for (int i = 0; i < length; i++)
				String [i] = reader [p++];
			return Encoding.UTF8.GetString(String, 0, length);
		} 
		...
		private static bool FindHead(byte[] reader, int attempts, ref int p)
		{//Attempts to find the "MThd" midi header in a stream
			int match = 0;
			while (attempts > 0)
			{
				switch ((char)reader[p++])
				{
				case 'M':
					match = 1;
					break;
