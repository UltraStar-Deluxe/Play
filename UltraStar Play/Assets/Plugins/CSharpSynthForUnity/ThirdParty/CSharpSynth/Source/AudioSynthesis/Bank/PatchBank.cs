...
namespace AudioSynthesis.Bank
{
    public class PatchBank
    {
		...
		public PatchBank(string bankFile)
		{
			bank = new Dictionary<int, Patch[]>();
			assets = new AssetManager();
			bankName = string.Empty;
		...
		{
			TextAsset asset = Resources.Load(bankFile) as TextAsset;
			...
			bankName = string.Empty;
			comment = string.Empty;
			switch (IOHelper.GetExtension(Path.GetFileName(bankFile).ToLower()))
			{
				case ".bank":
					bankName = IOHelper.GetFileNameWithoutExtension(Path.GetFileName(bankFile));
					LoadMyBank(stream);
				break;
