...
public class LrcFormatImporter : INeedInjection
{
    ...
    public List<Sentence> ImportLrcFormat(string lrcText, SongMeta songMeta, Settings settings)
    {
        ...
        for (int i = 0; i < parseResult.Lyrics.Lines.Count; i++)
        {
            ...
            Sentence sentence = ImportLrcLine(songMeta, beatsPerCharacter, settings, line, nextLine);
    ...
    private static Sentence ImportLrcLine(SongMeta songMeta, double beatsPerCharacter, Settings settings, Line line, Line nextLine)
    {
        ...
        return new Sentence(notesAfterSplit);
    }
}
